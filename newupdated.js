(function () {
                ...
                <div class="chatbox-header">
                    <div class="chatbox-header-1">
                        <button class="chatbox-header-btn expand-btn">
                            ...
                            <svg width="24" height="24" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" fill-rule="evenodd" clip-rule="evenodd" stroke-linecap="round" stroke-linejoin="round">
                             <g transform="matrix(1,0,0,-1,-2,22)"><path d="M15,3L21,3L21,9" style="fill:none;fill-rule:nonzero;stroke:currentColor;stroke-width:2px;"></path></g><g transform="matrix(1,0,0,-1,2,26)"><path d="M9,21L3,21L3,15" style="fill:none;fill-rule:nonzero;stroke:currentColor;stroke-width:2px;">
                            </path></g><g transform="matrix(0.857864,-0.142136,0.142136,-0.857864,0.558456,24.5585)"><path d="M21,3L14,10" style="fill:none;fill-rule:nonzero;stroke:currentColor;stroke-width:2px;"></path></g><g transform="matrix(0.858562,-0.141438,0.141438,-0.858562,-0.545877,23.4541)">
                            ...
                            <svg width="24" height="24" viewBox="0 0 24 24" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" fill-rule="evenodd" clip-rule="evenodd" stroke-linecap="round" stroke-linejoin="round">
                             <g transform="matrix(-1,0.000780613,0.000780613,1,33.9953,9.98595)"><path d="M15,3L21,3L21,9" style="fill:none;fill-rule:nonzero;stroke:currentColor;stroke-width:2px;"></path></g><g transform="matrix(-1,-0.000254637,-0.000254637,1,14.0046,-9.99847)">
                             <path d="M9,21L3,21L3,15" style="fill:none;fill-rule:nonzero;stroke:currentColor;stroke-width:2px;"></path></g><g transform="matrix(0.857864,-0.142136,0.142136,-0.857864,0.558456,24.5585)"><path d="M21,3L14,10" style="fill:none;fill-rule:nonzero;stroke:currentColor;stroke-width:2px;"></path></g>
                             <g transform="matrix(0.858562,-0.141438,0.141438,-0.858562,-0.545877,23.4541)"><path d="M3,21L10,14" style="fill:none;fill-rule:nonzero;stroke:currentColor;stroke-width:2px;"></path></g>
                             </svg>
                             <span>
                        </button>
                    ...
                    <div class="options-body">
                        <div class="options-section">
                            ...
                            <div class="color-picker-container">
                                ...
                                <input type="color" id="color-picker" value="${localStorage.getItem('chatWidgetBackground') || '#39B3BA'}">
                            </div>
                            <div class="color-picker-container">
                                <label for="bg-color-picker">Widget Text Color</label>
                                <input type="color" id="bg-color-picker" value="${localStorage.getItem('chatWidgetColor') || '#ffffff'}">
                            </div>
                        </div>
                        ...
                        <div class="options-section">
                            ...
                            <div class="toggle-container">
                                <label class="toggle">
                                    <input type="checkbox" id="sound-toggle" ${localStorage.getItem('soundEnabled') === 'true' ? 'checked' : ''}>
                        ...
                        <div class="options-section">
                            <h3>Chat History</h3>
                            <button class="options-action-btn download-transcript ${!hasMessages ? 'disabled' : ''}" ${!hasMessages ? 'disabled' : ''}>
    ...
    function initializeWidget() {
        ...
        const chatInput = container.querySelector('.chatbox-input');
        // const chatInputBtn = container.querySelector('.chatbox-input input');
        ...
        const maximize = container.querySelector('.maximize-icon');
        const minimize = container.querySelector('.minimize-icon');
        ...
        function updateBgColor(color) {
            const elementsToUpdate = document.querySelectorAll('.chat-button, .chatbox-header, .chatbox-header-btn, .send-button, .save-button, .icon');
            elementsToUpdate.forEach(el => el.style.backgroundColor = color);
        }

        function updateColor(color) {
            const elementsToUpdate = document.querySelectorAll('.chat-button, .chatbox-header, .chatbox-header-btn, .send-button, .save-button, .icon, .chatbox-header-3 ,.icon');
        ...
        function toggleOptions() {
            ...
            optionsPanel.style.height = optionsPanel.style.height === '0%' ? '100%' : '';
        }

        function toggleExpand() {
            chatbox.style.width = chatbox.style.width === '' ? '35vw' : '';
            chatbox.style.maxHeight = chatbox.style.maxHeight === '' ? '878px' : '';
            chatInput.style.width = chatInput.style.width === '96%' ? '94%' : '96%';
            maximize.style.display = maximize.style.display === 'block' ? 'none' : 'block';
            minimize.style.display = minimize.style.display === 'none' ? 'block' : 'none';
        ...
        async function initSocket() {
            try {
                ...
                if (!uniqueId) {
                    ...
                    const messages = await fetchPreviousMessages(uniqueId);
        ...
        async function initSession() {
            try {
                ...
                if (data.message === "success") {
                    ...
                    await getLocationDetails(uniqueId);
                }
                return data;
            } catch (error) {
        ...
        input.addEventListener('input', function () {
            sendButton.classList.toggle('visible', this.value.trim() !== '');
        });
        ...
        const notificationSound = new Audio('https://rajkumarmalluri.vercel.app/images/pyk-toon-n-n.mp3'); // Add your notification sound file
        ...
        function addMessage(text, type, username = '') {
            ...
            // Update hasMessages state and download button
            hasMessages = true;
            if (elements.downloadTranscriptBtn) {
                elements.downloadTranscriptBtn.classList.remove('disabled');
        ...
        document.querySelector('.download-transcript').addEventListener('click', function () {
